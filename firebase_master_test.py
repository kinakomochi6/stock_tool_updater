    ...
    def fetch_list(
        ...
        for i in range(days_back + 1):
            ...
            for attempt in range(1, 4):
                try:
                    ...
                    try:
                        ...
                        body_preview = str(getattr(res, "text", ""))[:120].replace("\n", " ")
                        raise RuntimeError(
                            "JSON解析失敗 "
                            f"(Content-Type={content_type or '不明'}, body={body_preview!r})"
                        ) from exc
                    if "results" not in js:
                        api_status = js.get("StatusCode", "不明")
                        api_message = str(js.get("message", ""))[:120]
                        raise RuntimeError(
                            "results field is missing "
                            f"(StatusCode={api_status}, message={api_message!r})"
                        )
                    break
                except Exception as exc:
            ...
            if js is None:
                ...
                if not self.successful_fetch_dates and consecutive_failures >= 3:
                    self.df_docs = pd.DataFrame(all_docs)
                    raise RuntimeError(
                        "EDINET書類一覧の取得に3日連続で失敗したため、"
                        "既存データを保護して処理を中止します。"
                        f" 最終エラー: {last_error}"
                    )
                continue
            ...
            consecutive_failures = 0
            self.successful_fetch_dates.append(target_date.strftime('%Y-%m-%d'))
            count = 0
        ...
        self.df_docs = pd.DataFrame(all_docs)
        if not self.successful_fetch_dates:
            latest_error = self.fetch_failures[-1]["error"] if self.fetch_failures else "応答なし"
            raise RuntimeError(
                "EDINET書類一覧を1日分も取得できなかったため、既存データを保護して処理を中止します。"
                f" 最終エラー: {latest_error}"
            )
        print(f"\n--- 完了: 合計 {len(self.df_docs)} 件の書類情報を確保 ---")

    def can_conclude_document_missing(self):
        """Return true only when the scanned date range has no retrieval gaps."""
        return bool(self.successful_fetch_dates) and not self.fetch_failures
...
def get_all_listed_codes():
    ...
    try:
        ...
        if not content.startswith(b"PK"):
            raise RuntimeError("JPX応答がExcelファイルではありません")
        ...
        if not companies:
            raise RuntimeError("JPX銘柄一覧から普通株式を1件も抽出できませんでした")
        return companies
    except Exception as e:
        raise RuntimeError(f"上場銘柄リストの取得に失敗しました: {e}") from e
    ...
    for i, code in enumerate(target_codes, 1):
        ...
        try:
            ...
            if bs_doc_id:
                ...
                if searcher.can_conclude_document_missing():
                    document_status, source_note = classify_missing_bs_document(
                        market, bs_desc
                    )
                    bs_quality["reasons"] = [source_note or bs_desc or document_status]
                else:
                    bs_validation_conclusive = False
                    document_status = "source_incomplete"
                    source_note = "EDINET書類一覧の取得欠損があるため、書類未発見を確定できません"
                if args.debug_bs:
                    debug_path = write_bs_diagnostics(args.debug_dir, code, {
